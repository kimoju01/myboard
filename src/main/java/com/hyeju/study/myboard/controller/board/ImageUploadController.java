...
//    public void uploadImage(@RequestParam("file") MultipartFile multipartFiles) {
    public ResponseEntity<?> handleImageUpload(@RequestParam("file") MultipartFile multipartFile) {
        try {
            return ResponseEntity.ok().body("/image/" + imageService.upload(multipartFile));
